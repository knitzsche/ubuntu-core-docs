...
Ubuntu Core 18 introduces *bases*. A base provides a rootfs snap (either "core" or "core18). An Ubuntu Core 18 image incldues the "core18" snap, and an Ubuntu Core 16 image contains the "core" snap.

An application snap can specify its base. If the base is not present, it is installed. For example, if you installed an app snap that specifies a core18 base on a core16 system, the core18 snap is installed, and that snap uses the core18 snap's rootfs as it's runtime root file system.

TODO add link to snapcraft.io
